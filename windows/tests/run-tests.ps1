...
try {
  ...
  if ([regex]::Matches($installSource, '-ExecutionPolicy RemoteSigned').Count -ne 2 -or
    ...
    throw 'Installer launch/restore shortcuts still bypass the PowerShell execution policy.'
  }
  if ($installSource.Contains("CreateShortcut((Join-Path `$folder 'Codex Kanna Blue Skin - Tray.lnk'))") -or
    $installSource.Contains('Start-Process -FilePath $powershell') -or
    -not $installSource.Contains("'Codex Kanna Blue Skin - Tray.lnk'")) {
    throw 'Installer still creates/auto-starts the hidden PowerShell tray shortcut or no longer removes its legacy copy.'
  }
  ...
  if ($initialTheme.Theme.id -cne 'preset-kanna-hashimoto' -or
    $initialTheme.Theme.name -cne '桥本环奈 · 蓝色瞬间' -or
    ...
    throw 'Default Windows theme did not seed the Kanna Hashimoto wallpaper contract.'
  }
  ...
  if ($preseededThemes.Count -lt 3 -or
    $preseededIds -notcontains 'preset-kanna-hashimoto' -or
    $preseededIds -notcontains 'preset-gothic-void-crusade' -or
    $preseededIds -notcontains 'preset-milky-way-glass') {
    throw 'Windows did not preseed Kanna Hashimoto, Gothic Void Crusade, and Milky Way Glass.'
  }
  ...
  $milkyWaySeed = $preseededThemes | Where-Object { $_.Id -ceq 'preset-milky-way-glass' } | Select-Object -First 1
  $milkyWayLoaded = if ($null -ne $milkyWaySeed) {
    Read-DreamSkinTheme -ThemeDirectory $milkyWaySeed.Path
  } else { $null }
  if ($null -eq $milkyWayLoaded -or $milkyWaySeed.Name -cne '银河夜幕 · 星河玻璃' -or
    $milkyWayLoaded.Theme.appearance -cne 'dark' -or
    $milkyWayLoaded.Theme.surfaces.sidebarBackground -cne 'continuous' -or
    $milkyWayLoaded.Theme.surfaces.matchWorkspaceTransparency -ne $true -or
    $milkyWayLoaded.Theme.materials.dark.page -ne 0.30 -or
    $milkyWayLoaded.Theme.materials.components.menus.dark.opacity -ne 0.58) {
    throw 'Milky Way Glass was not preseeded with the expected dark translucent material contract.'
  }
  $updatedTheme = Set-DreamSkinActiveTheme -ImagePath (Join-Path $Root 'assets\dream-reference.png') `
  ...
  Copy-Item -LiteralPath (Join-Path $Root 'assets\dream-reference.png') `
    -Destination (Join-Path $outsideTheme 'dream-reference.png')
  ...
  foreach ($requiredRendererBehavior in @(
    'dream-home-shell', 'dream-settings-shell', 'dream-utility-shell', 'dream-task-shell',
  ...
  $cdpSessionBehaviorTest = Invoke-DreamSkinNative -FilePath $dotnet -ArgumentList @(
    'run',
    '--project', (Join-Path $PSScriptRoot 'CdpSessionBehavior\CdpSessionBehavior.csproj'),
    '--configuration', 'Release',
    '--nologo'
  )
  if ($cdpSessionBehaviorTest.ExitCode -ne 0 -or
    ($cdpSessionBehaviorTest.Output -join "`n") -notmatch 'PASS: CdpSession cancellation and disconnect cleanup behavior\.') {
    throw "CdpSession behavior test failed: $($cdpSessionBehaviorTest.Output -join '<NL>')"
  }
