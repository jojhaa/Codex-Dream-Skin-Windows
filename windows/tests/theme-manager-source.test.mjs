...
for (const token of [
  ...
  assert.ok(project.includes(token), `missing v0.3.8 assembly contract: ${token}`);
assert.ok(manifest.includes('Version="0.3.8.0"'),
  "the packaged identity must use v0.3.8");
assert.ok(releaseChecks.includes('UserAgent.ParseAdd("CodexDreamSkin/0.3.8")'),
  "release checks must identify the v0.3.8 client");
