...
function Initialize-DreamSkinThemeStore {
  ...
  $assetImage = Join-Path $assetRoot 'dream-reference.png'
  ...
  if (-not (Test-Path -LiteralPath $activeTheme -PathType Leaf)) {
    ...
    Assert-DreamSkinNoReparseComponents -Path (Join-Path $paths.Active 'dream-reference.png')
    $activeImage = Join-Path $paths.Active 'dream-reference.png'
    ...
    $imageArchive = Join-Path $paths.Images 'dream-reference.png'
  ...
  $retiredArinaPresetDirectory = Join-Path $paths.Saved 'preset-arina-hashimoto'
  Assert-DreamSkinNoReparseComponents -Path $retiredArinaPresetDirectory
  if (Test-Path -LiteralPath $retiredArinaPresetDirectory) {
    Remove-Item -LiteralPath $retiredArinaPresetDirectory -Recurse -Force
  }
  # Bundled Kanna Hashimoto theme (the default active wallpaper lives under assets/).
  $presetDirectory = Join-Path $paths.Saved 'preset-kanna-hashimoto'
  ...
  if (-not (Test-Path -LiteralPath $presetTheme -PathType Leaf)) {
    ...
    $presetImage = Join-Path $presetDirectory 'dream-reference.png'
